...
import { completedTodo } from "../redux/addTodoSlice";
import { removeTodo } from "../redux/addTodoSlice";
...
function TodoItem({ content, completed, id }) {
  ...
  return (
    <div className="todo__container">
      <div
        className={`circle__todo ${completed ? "active" : ""}`}
        onClick={completedTodoHandler}
      >
        <img alt="" />
      </div>
      <li className={`todo ${completed ? "active" : ""}`}>{content}</li>
      <div className="btn" onClick={removeTodoHandler}>
        <svg
